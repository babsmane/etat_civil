...
describe('Region e2e test', () => {
  ...
  it('should load details Region page', () => {
    cy.intercept('GET', '/api/regions*').as('entitiesRequest');
    cy.visit('/');
    cy.clickOnEntityMenuItem('region');
    cy.wait('@entitiesRequest');
  ...
  it('should load edit Region page', () => {
    cy.intercept('GET', '/api/regions*').as('entitiesRequest');
    cy.visit('/');
    cy.clickOnEntityMenuItem('region');
    cy.wait('@entitiesRequest');
  ...
  it('should create an instance of Region', () => {
    ...
    cy.get(`[data-cy="regionCode"]`).type('19050').should('have.value', '19050');

    cy.get(`[data-cy="regionName"]`).type('a reciprocal', { force: true }).invoke('val').should('match', new RegExp('a reciprocal'));

    cy.get(entityCreateSaveButtonSelector).click({ force: true });
    cy.scrollTo('top', { ensureScrollable: false });
    cy.get(entityCreateSaveButtonSelector).should('not.exist');
    cy.intercept('GET', '/api/regions*').as('entitiesRequestAfterCreate');
    cy.visit('/');
    cy.clickOnEntityMenuItem('region');
  ...
  it('should delete last instance of Region', () => {
    cy.intercept('GET', '/api/regions*').as('entitiesRequest');
    ...
    cy.wait('@entitiesRequest').then(({ request, response }) => {
      ...
      if (startingEntitiesCount > 0) {
        ...
        cy.intercept('GET', '/api/regions*').as('entitiesRequestAfterDelete');
        cy.visit('/');
        cy.clickOnEntityMenuItem('region');
