...
/**
 * REST controller for managing {@link sn.architech.etatcivil.domain.Commune}.
 */
@RestController
@RequestMapping("/api")
public class CommuneResource {
    ...
    public ResponseEntity<Commune> updateCommune(@PathVariable(value = "id", required = false) final Long id, @RequestBody Commune commune)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, commune.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Commune> partialUpdateCommune(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, commune.getId().toString())
        );
    }
